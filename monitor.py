import requests
from bs4 import BeautifulSoup
import json
import os
from datetime import datetime
import re

def get_latest_versions():
    versions = {}
    header = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    }
    timeout_sec = 45

    # 1. Chrome (API)
    try:
        chrome_api = "https://versionhistory.googleapis.com/v1/chrome/platforms/win/channels/stable/versions"
        c_data = requests.get(chrome_api, timeout=timeout_sec).json()
        versions['chrome'] = {"version": c_data['versions'][0]['version'], "date": datetime.now().strftime("%Y/%m/%d")}
    except: pass

    # 2. Microsoft Edge (Relnote 경로 패턴 매칭)
    edge_url = "https://learn.microsoft.com/en-us/deployedge/microsoft-edge-relnote-stable-channel"
    try:
        res = requests.get(edge_url, headers=header, timeout=timeout_sec)
        # 이미지 힌트 확인 결과: Version 148.0.3967.54 가 최우선 텍스트임
        match = re.search(r'Version\s+(\d{3}\.0\.\d{4}\.\d{2})', res.text)
        if match:
            versions['edge'] = {"version": match.group(1), "date": datetime.now().strftime("%Y/%m/%d")}
    except: pass

    # 3. Bandizip (이미지 속 ld+json 정밀 추출)
    bandi_url = "https://www.bandisoft.com/bandizip/history/"
    try:
        res = requests.get(bandi_url, headers=header, timeout=timeout_sec)
        soup = BeautifulSoup(res.text, 'html.parser')
        # 스크린샷 1번의 <script type="application/ld+json"> 부분 추출
        json_script = soup.find('script', type='application/ld+json')
        if json_script:
            data = json.loads(json_script.string)
            # JSON 구조 내 softwareVersion 및 datePublished 활용
            versions['bandizip'] = {
                "version": data.get('softwareVersion', 'v7.43'),
                "date": data.get('datePublished', '2026/05/04')
            }
    except: pass

    # 4. Adobe Acrobat (이미지 속 <link rel="next"> 태그 정밀 추출)
    acrobat_url = "https://www.adobe.com/devnet-docs/acrobatetk/tools/ReleaseNotesDC/index.html"
    try:
        res = requests.get(acrobat_url, headers=header, timeout=timeout_sec)
        soup = BeautifulSoup(res.text, 'html.parser')
        # 스크린샷 2번의 <link rel="next" title="26.001.21529 ..."> 부분 활용
        link_tag = soup.find('link', rel='next')
        if link_tag and 'title' in link_tag.attrs:
            title_text = link_tag['title'] # "26.001.21529 Planned update, May 01, 2026"
            v_match = re.search(r'(\d{2}\.\d{3}\.\d{5})', title_text)
            d_match = re.search(r'([a-zA-Z]+\s+\d+,\s+\d+)', title_text)
            if v_match:
                versions['acrobat'] = {
                    "version": v_match.group(1),
                    "date": d_match.group(1) if d_match else "May 01, 2026"
                }
    except: pass

    return versions

def send_telegram_msg(message):
    token = os.environ.get('TELEGRAM_TOKEN')
    chat_id = os.environ.get('TELEGRAM_CHAT_ID')
    if not (token and chat_id): return
    url = f"https://api.telegram.org/bot{token}/sendMessage"
    try: requests.get(url, params={"chat_id": chat_id, "text": message}, timeout=10)
    except: pass

def main():
    json_path = 'versions.json'
    if not os.path.exists(json_path):
        with open(json_path, 'w') as f: json.dump({}, f)

    with open(json_path, 'r', encoding='utf-8') as f:
        try: old_data = json.load(f)
        except: old_data = {}

    new_data = get_latest_versions()
    changed = False
    message = "🔔 [S/W 보안 업데이트 정밀 감지]\n\n"

    for name in ['chrome', 'edge', 'bandizip', 'acrobat']:
        if name in new_data:
            # 기존 데이터 형식(문자열/객체) 호환
            old_v_info = old_data.get(name, {})
            old_v = old_v_info.get('version', "0.0.0") if isinstance(old_v_info, dict) else old_v_info
            new_v = new_data[name]['version']

            if old_v != new_v:
                message += f"✅ {name.upper()}\n- 버전: {old_v} → {new_v}\n- 날짜: {new_data[name]['date']}\n\n"
                old_data[name] = new_data[name]
                changed = True

    if changed:
        send_telegram_msg(message)
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(old_data, f, indent=2, ensure_ascii=False)
    else:
        print(f"[{datetime.now()}] 모든 소프트웨어 최신 상태")

if __name__ == "__main__":
    main()
